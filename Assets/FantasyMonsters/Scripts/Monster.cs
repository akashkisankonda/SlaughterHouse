...
namespace Assets.FantasyMonsters.Scripts
{
    ...
    public class Monster : MonoBehaviour
    {
        ...
        private float mouseDownTimeStamp = 0;
        private float tabThreshold = 0.1f;
        private bool isClickable = false;
        private GoatStore gs;
        private int index;
        public void SetStoreItem(GoatStore store, int i)
        {
            isClickable = true;
            gs = store;
            index = i;
        ...
        void OnMouseUp()
        {
            ...
            if(Time.time - mouseDownTimeStamp < tabThreshold)
            {
                Debug.Log("Clicked on " + gameObject.name);
                bool result = gs.Purchase(index);
                if (!result) return;

                transform.Find("coim(Clone)").gameObject.SetActive(false);
                transform.Find("CostText").GetComponent<TextMesh>().text = "";

                transform.Find("tagg").gameObject.SetActive(true);
            }
        }
